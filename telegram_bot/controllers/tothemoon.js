  ...
  const interval = setInterval(async () => {
    ...
    for (const bmarkets of Object.values(binancemarkets)) {
      if (
        ...
        try {
          const result = await Cex.findOneAndUpdate(
            { pairAddress: pairAddress },
            { $set: update },
...
exports.getDEX = async (req, res, next) => {
  ...
  const interval = setInterval(async () => {
    ...
    for (let i = 0; i < l; i++) {
      const url = `http://api.dexscreener.com/latest/dex/pairs/${
        tokens[(cnt + i) % total]["chainId"]
      }/${tokens[(cnt + i) % total]["pairAddress"]}`;
      urls.push(url);
    ...
    try {
      axios.all(requests).then(async (res) => {
        for (let j = 0; j < l; j++) {
          ...
          try {
            let res = await Dex.findOneAndUpdate(
              { pairAddress: pairAddress },
              { $set: update },
...
exports.getArbitrage = async (req, res, next) => {
  ...
  for (const a in arbitrage) {
    if (arbitrage[a].length > 1) {
      // needs more than one pair to have arbitrage
      arbitrage[a].sort((a, b) => b.priceUsd - a.priceUsd);
      const l = arbitrage[a].length - 1;
      const percentage =
        ((arbitrage[a][0].priceUsd - arbitrage[a][l].priceUsd) * 100) /
        arbitrage[a][l].priceUsd;
